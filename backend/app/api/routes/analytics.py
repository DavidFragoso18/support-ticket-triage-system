...
def get_overview(
    ...
    try:
        ...
        # Average confidence
        avg_confidence = (
            session.exec(select(func.avg(TicketClassification.confidence))).one() or 0.0
        )

        # Low confidence count
        low_confidence_count = session.exec(
            select(func.count(TicketClassification.id)).where(TicketClassification.low_confidence)
        ).one()

        # Feedback count
        feedback_count = session.exec(select(func.count(ClassificationFeedback.id))).one()
...
def get_intent_distribution(
    ...
    try:
        ...
        results = session.exec(
            select(
                TicketClassification.intent, func.count(TicketClassification.id).label("count")
        ...
        raise internal_error(
            "GET_INTENT_DISTRIBUTION_FAILED", "Could not retrieve intent distribution."
        )
...
def get_sentiment_distribution(
    ...
    try:
        results = session.exec(
            select(
                TicketClassification.sentiment, func.count(TicketClassification.id).label("count")
        ...
        raise internal_error(
            "GET_SENTIMENT_DISTRIBUTION_FAILED", "Could not retrieve sentiment distribution."
        )
...
def get_priority_distribution(
    ...
    try:
        results = session.exec(
            select(
                TicketClassification.priority, func.count(TicketClassification.id).label("count")
        ...
        raise internal_error(
            "GET_PRIORITY_DISTRIBUTION_FAILED", "Could not retrieve priority distribution."
        )
...
def get_tickets_over_time(
    ...
    try:
        ...
        results = session.exec(
            select(func.date(Ticket.created_at).label("date"), func.count(Ticket.id).label("count"))
        ...
        if not results:
            ...
            results = session.exec(
                select(
                    func.date(Ticket.created_at).label("date"), func.count(Ticket.id).label("count")
                )
                .group_by(func.date(Ticket.created_at))
        ...
        return [TimeSeriesData(date=str(date), count=count) for date, count in results]
    except Exception:
...
def get_classification_accuracy(
    ...
    try:
        ...
        intent_accepted = sum(
            ...
            if f.action == "accepted" or (f.action == "corrected" and f.corrected_intent is None)
        )
        intent_corrected = sum(
            1 for f in all_feedback if f.action == "corrected" and f.corrected_intent is not None
        )
        ...
        sentiment_accepted = sum(
            1
            for f in all_feedback
            if f.action == "accepted" or (f.action == "corrected" and f.corrected_sentiment is None)
        )
        sentiment_corrected = sum(
            1 for f in all_feedback if f.action == "corrected" and f.corrected_sentiment is not None
        )
        ...
        priority_accepted = sum(
            1
            for f in all_feedback
            if f.action == "accepted" or (f.action == "corrected" and f.corrected_priority is None)
        )
        priority_corrected = sum(
            1 for f in all_feedback if f.action == "corrected" and f.corrected_priority is not None
        )
        ...
        raise internal_error(
            "GET_CLASSIFICATION_ACCURACY_FAILED", "Could not retrieve accuracy metrics."
        )
...
def get_agent_performance(
    ...
    try:
        ...
        agents = session.exec(
            select(AgentActivity.agent_id).where(AgentActivity.timestamp >= start_date).distinct()
        ).all()
        ...
        raise internal_error(
            "GET_AGENT_PERFORMANCE_FAILED", "Could not retrieve agent performance data."
        )
