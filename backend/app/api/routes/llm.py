...
async def suggest_response(
    ticket_id: str,
    tone: str = Query(
        default="professional", regex="^(professional|friendly|technical|empathetic)$"
    ),
    ...
    try:
        ...
        response_text = await llm_service.generate_response(
            ticket_subject=ticket.subject, ticket_body=ticket.body, context=context, tone=tone
        )
...
async def _build_rag_context(subject: str, body: str, session: Session, max_items: int = 5) -> list:
    """
    ...
    try:
                ...
                context.append(
                    {
                        ...
                        "data": {"id": str(row[0]), "subject": row[1], "body": row[2]},
                    }
                )
        ...
        for row in kb_results:
            context.append(
                {"type": "kb", "data": {"id": str(row[0]), "title": row[1], "body": row[2]}}
            )
        ...
        for row in res_results:
            context.append(
                {"type": "resolution", "data": {"id": str(row[0]), "title": row[1], "body": row[2]}}
            )
