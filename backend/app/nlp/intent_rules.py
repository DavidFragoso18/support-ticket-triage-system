...
_INTENT_PATTERNS: dict[str, list[re.Pattern]] = {
    ...
    "bug_issue": [
        ...
        re.compile(r"\b(app\s+crash|won'?t\s+(play|load|sync)|notifications?\s+not\s+working)\b"),
        re.compile(r"\b(barcode\s+scanner|video\s+won'?t\s+play|cache|offline)\b"),
    ],
    "usage_howto": [
